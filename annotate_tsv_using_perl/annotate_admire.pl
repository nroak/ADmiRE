...
Download ADmiRE.tab from: https://figshare.com/s/7b3bea2aa33470396f50

Run the script: 
perl annotate_admire.pl [--input FILE] [--output FILE] [--admire_path=PATH] [--chr=NUMBER] [--pos=NUMBER]
--input 		  = Tab separated input file
--output [OPTIONAL]	  = 17 columns from admire database are added to the end of each row, if no information is available, 'NA's are populated, 
		    	    Default file name, $input.admire.tab
--admire_path [OPTIONAL]  = File location to ADmiRE.tab file. Default is the same directory as the code.
--chr 			  = Input the COLUMN number with chromosome information from the tab separated file
--pos 			  = Input the COLUMN number with position information from the tab separated file
...
ADmiRE Headers												
   ...
   9  Mature_Type											
  10  High_Confidence											
  11  gnomAD_Constrained_MIRNA										
  12  Conserved_ADmiRE											
  13  HMDD_Disease (PMID)										
  14  "HMDD_Targets (Disease, PMID)"									
  15  miRTarBase_#Validated Targets									
  16  miRTarBase_TargetGene(Validation type)								
  17  miRTarBase_Reference(PMID)									
...
ALLMUT: while (my $line_maf = <FP>){
	...
	chomp @entry_maf;
	if ($line_maf =~ /^\#/ || $line_maf =~ /^CHROM/) { print FO "@entry_maf\tMIRNA\tPRE_ID\tMIRNA_Feature\tFamily_Name\tFamily_ID\tMature_Pos\tMature_Name\tMature_ID\tMature_Type\tHigh_Confidence\tgnomAD_Constrained_MIRNA\tConserved_100vert_ADmiRE\tHMDD_Disease (PMID)\tHMDD_Targets (Disease, PMID)\tmiRTarBase_#Validated Targets\tmiRTarBase_TargetGene(Validation type)\tmiRTarBase_Reference(PMID)\n";}
	elsif ($line_maf !~ /^\#/ && $line_maf !~ /^CHROM/) {
		my $db_key=$entry_maf[$chr]."_".$entry_maf[$pos];
		if (!defined %{$mirbase{$db_key}}){print FO "@entry_maf\t@empty\n";next ALLMUT;}
