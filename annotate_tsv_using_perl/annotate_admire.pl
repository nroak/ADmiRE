...
ADmiRE Headers												
    ...
    16  Conserved_ADmiRE
    17  HMDD_Disease (PMID)
    18  HMDD_Targets (Disease, PMID)
    19  miRTarBase_#Validated Targets
    20  miRTarBase_TargetGene(Validation type)
    21  miRTarBase_Reference(PMID)
    22  Phylop_100way
    23  Phastcons_100way
    24  gnomAD_Count
    25  gnomAD_MAF
...
ALLMUT: while (my $line_maf = <FP>){
	...
	chomp @entry_maf;
	if ($line_maf =~ /^\#/ || $line_maf =~ /^CHROM/ || $line_maf =~ /^Hugo_Symbol/) { print FO "@entry_maf\tMIRNA\tPRE_ID\tMIRNA_Feature\tFamily_Name\tFamily_ID\tPrecursor_Pos\tMature_Pos\tMature_Name\tMature_ID\tMature_Type\tHigh_Confidence\tgnomAD_Constrained_MIRNA\tConserved_100vert_ADmiRE\tHMDD_Disease (PMID)\tHMDD_Targets (Disease, PMID)\tmiRTarBase_#Validated Targets\tmiRTarBase_TargetGene(Validation type)\tmiRTarBase_Reference(PMID)\tPhyloP_100way\tPhastcons_100way\tgnomAD_Count\tgnomAD_MAF\n";}
		...
		foreach my $db_key2 ( keys %{$mirbase{$db_key}}) {
			print FO "@entry_maf\t@{$mirbase{$db_key}{$db_key2}}[3..24]\n";
		   	}
		next ALLMUT;
		}
	}
